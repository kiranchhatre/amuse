...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        # dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=x.device) # TODO: check x.device usage in global context
...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor_list):
        x = tensor_list.tensors
        h, w = x.shape[-2:]
        # i = torch.arange(w, device=x.device) # TODO: check x.device usage in global context
        # j = torch.arange(h, device=x.device) # TODO: check x.device usage in global context
        i = torch.arange(w)
        j = torch.arange(h)
