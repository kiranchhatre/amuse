...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l2_loss_new = th.nn.MSELoss(reduction='none')                      # Modified
    ...
    def simple_l2(self, output, target):
        assert output.keys() == target.keys(), "output and target must have the same keys!"
        loss = {}
        for k in output.keys():
            ...
            _loss = sum_flat(self.l2_loss(output[k], target[k]))
    ...
    def q_posterior_mean_variance_modified(self, x_start, x_t, t):
        ...
        """
        if self.dis_repr:
            assert x_start["pose_content"].shape == x_t["pose_content"].shape
            assert x_start["pose_emotion"].shape == x_t["pose_emotion"].shape
        else:
            assert x_start["pose_combo"].shape == x_t["pose_combo"].shape
        ...
        for k in x_start.keys():
            posterior_mean[k], posterior_variance[k], posterior_log_variance_clipped[k] = \
                self.q_posterior_mean_variance(x_start[k], x_t[k], t)
    ...
    def p_mean_variance(
        ...
        self.arb_val = next(iter(x.values()))
        B, C = self.arb_val.shape[0], self.arb_val.shape[-1]
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        
        # Inpainting not supported for now
        # model_kwargs['y'] = {} # Empty placeholder for inpainting
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # model_variance = _extract_into_tensor(model_variance, t, x.shape)                 # Obsolete
            # model_log_variance = _extract_into_tensor(model_log_variance, t, x.shape)         # Obsolete
            
            if self.dis_repr:
                con_model_variance = _extract_into_tensor(model_variance, t, x["pose_content"].shape)
                con_model_log_variance = _extract_into_tensor(model_log_variance, t, x["pose_content"].shape)
                emo_model_variance = _extract_into_tensor(model_variance, t, x["pose_emotion"].shape)
                emo_model_log_variance = _extract_into_tensor(model_log_variance, t, x["pose_emotion"].shape)
                model_variance = {"pose_content": con_model_variance, "pose_emotion": emo_model_variance}
                model_log_variance = {"pose_content": con_model_log_variance, "pose_emotion": emo_model_log_variance}
            else:
                comb_model_variance = _extract_into_tensor(model_variance, t, x["pose_combo"].shape)
                comb_model_log_variance = _extract_into_tensor(model_log_variance, t, x["pose_combo"].shape)
                model_variance = {"pose_combo": comb_model_variance}
                model_log_variance = {"pose_combo": comb_model_log_variance}
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:
                ...
                pred_xstart = pred_xstart["recons"]
            else:
                pred_xstart = process_xstart(
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
                )
            # model_mean, _, _ = self.q_posterior_mean_variance(                # Obsolete
        ...
        if self.dis_repr:
            assert (
                model_mean["pose_content"].shape == model_log_variance["pose_content"].shape == pred_xstart["pose_content"].shape == x["pose_content"].shape
            )
            assert (
                model_mean["pose_emotion"].shape == model_log_variance["pose_emotion"].shape == pred_xstart["pose_emotion"].shape == x["pose_emotion"].shape
            )
        else:
            assert (
                model_mean["pose_combo"].shape == model_log_variance["pose_combo"].shape == pred_xstart["pose_combo"].shape == x["pose_combo"].shape
            )
    ...
    def p_sample(
        self,
        model,
        x,                                                                      # images dict from p_sample_loop_progressive
        t,
        clip_denoised=True,
        denoised_fn=None,
        cond_fn=None,
        ...
        # noise = th.randn_like(x)                                              # Obsolete
        if self.dis_repr:
            noise_con = th.randn_like(x["pose_content"])
            noise_emo = th.randn_like(x["pose_emotion"])
            noise = {"pose_content": noise_con, "pose_emotion": noise_emo}
        else:
            noise_comb = th.randn_like(x["pose_combo"])
            noise = {"pose_combo": noise_comb}
        
        if const_noise:
            noise = noise[[0]].repeat(x.shape[0], 1, 1, 1)

        nonzero_mask = (                                                        # no noise when t == 0
            (t != 0).float().view(-1, *([1] * (len(self.arb_val.shape) - 1)))
        )  
        ...
        out_formed = out["pred_xstart"]
        sample_formed = {}
        for key in noise.keys():
            sample_formed[key] = out["mean"][key] + nonzero_mask * th.exp(0.5 * out["log_variance"][key]) * noise[key]
        
        # sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise   # Obsolete
        # return {"sample": sample, "pred_xstart": out["pred_xstart"]}                      # Obsolete
        
        return {"sample": sample_formed, "pred_xstart": out_formed}                         # both are dicts
    ...
    def p_sample_loop(
        self,
        model,
        shape,                                                                  # all_motion from diffusion_eval.py
        noise=None,
        clip_denoised=True,
        denoised_fn=None,
        cond_fn=None,
    ...
    def p_sample_loop_progressive(
        ...
        self.dis_repr = True if "pose_content" in shape.keys() else False  
        # assert isinstance(shape, (tuple, list))
        if noise is not None:
            img = noise
        else:
            if self.dis_repr:
                img_con = th.randn(*shape["pose_content"].shape, device=device)
                img_emo = th.randn(*shape["pose_emotion"].shape, device=device)
                images = {"pose_content": img_con, "pose_emotion": img_emo}
                bs = shape["pose_content"].shape[0]
            else:
                img_combo = th.randn(*shape["pose_combo"].shape, device=device)
                images = {"pose_combo": img_combo}
                bs = shape["pose_combo"].shape[0]
        ...
        for i in indices:
            # t = th.tensor([i] * shape[0], device=device)                      # Obsolete
            t = th.tensor([i] * bs, device=device)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None, compose_in_bvhfac=False, mode=""):
        """
        ...
        Modified for dis_s2g as:
        
        :param model: <models.diffusion.utils.mdm_respace._WrappedModel object>
        :param x_start: dict_keys(['pose_content', 'pose_emotion', 'pose_combo'])
        :param t: torch.Size([64])
        :param model_kwargs: dict_keys(['wav_mfcc_content', 'wav_mfcc_emotion', 'wav_mfcc', 'attr', 'corpus_gpt', 'emo_label']
        :param noise: None
        :param dataset: None
        :return:
        """
        ...
        mask = None                                                             # KIRAN TODO
        self.mode = mode                                                        # Empty placeholder            
        if not compose_in_bvhfac: 
            if model_kwargs is None:
                model_kwargs = {}
            if noise is None:
                noise = th.randn_like(x_start)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if compose_in_bvhfac:
                model_output = model(x_start, self._scale_timesteps(t), **model_kwargs)
                assert all([v==None for v in model_output["losses"].values()]), "Losses should be None at this point"
                model_output = model_output["recons"]
            else: model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            ...
            if not compose_in_bvhfac:
                ...
                assert self.model_mean_type == ModelMeanType.START_X, "Only ModelMeanType.START_X is supported for BVHfac"
                target = x_start

            # terms["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
            terms["rot_mse"] = self.simple_l2(output=model_output, target=target) 

            if not compose_in_bvhfac:                                           # Not used for BVHfac
