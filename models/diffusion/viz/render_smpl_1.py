...
class BlenderVisualizer():
    ...
    def __init__(self, args):
        ...
        if not self.modified_smplx_viz:                     # FBX based
            ...
            if self.person in males:
                ...
                self.body_location = (0,0,-1.19 )
                self.mesh_name = "SMPLX-mesh-male"
            elif self.person in females:
                self.floor_location = (0, 0, -1.13961)
                self.body_location = (0,0,-1.3)
                self.mesh_name = "SMPLX-mesh-female"
            else: int(f"Unknown person: {self.person}")
    ...
    def render_clip(self):
        ...
        # light above head
        bpy.ops.object.light_add(type='AREA', align='WORLD', location=(0, 0, 1), scale=(1, 1, 1))
        ...
        if not self.modified_smplx_viz:
            ...
            armature.location = self.body_location
        ...
        # wall
        # bpy.ops.mesh.primitive_plane_add(size=5, enter_editmode=False, align='WORLD', location=(0, 0.5, 0), rotation=(radians(90), 0, 0))
        bpy.ops.mesh.primitive_plane_add(size=5, enter_editmode=False, align='WORLD', location=(0, 1.5, 0), rotation=(radians(90), 0, 0))
        ...
        self._render_setting(self.args.endFrame)
...
if __name__ == "__main__":
    assert len(sys.argv) == 10, "[Blender] Invalid number of arguments"
    try:
